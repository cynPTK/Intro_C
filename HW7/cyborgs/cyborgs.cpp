...
void Cyborg::forceMove(int dir)
{
    ...
    if (!isDead())
    {
        ...
        if (r < 1 || r > m_arena->rows() || c < 1 || c > m_arena->cols() || m_arena->hasWallAt(r, c))
        {
            m_health--;
        }
        else
...
string Player::move(int dir)
{
    ...
    case NORTH:
        r--;
        retstr = "Player moved north.";
        break;
    case SOUTH:
        r++;
        retstr = "Player moved south.";
        break;
    case EAST:
        c++;
        retstr = "Player moved east.";
        break;
    case WEST:
        c--;
        retstr = "Player moved west.";
        break;
    }
    ...
    if (m_arena->numberOfCyborgsAt(r, c) > 0)
    {
        setDead();
    ...
    for (int i = 0; i < m_nCyborgs; i++)
    {
        if (m_cyborgs[i] != nullptr)
        {
            ...
            if (m_cyborgs[i]->isDead())
            {
                ...
                for (int k = i + 1; k < m_nCyborgs; k++)
                {
                    m_cyborgs[k] = m_cyborgs[k + 1];
                }
                m_nCyborgs--;
            }
        }
...
bool attemptMove(const Arena& a, int dir, int& r, int& c)
{
    ...
    if (row < 1 || row > a.rows() || col < 1 || col > a.rows() || a.hasWallAt(row,col))
    {
        return false; 
    }
    else
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    ...
    int standing = bestDir;
    int row = r;
    int col = c;
    int minCyborgCount = a.cyborgCount();
    bestDir = -1;
    ...
    for (int i = -1; i < 4; i++)
    {
        ...
        int cyborgCount =
            a.numberOfCyborgsAt(row + 1, col) +
            a.numberOfCyborgsAt(row - 1, col) +
            a.numberOfCyborgsAt(row, col + 1) +
            a.numberOfCyborgsAt(row, col - 1) +
            a.numberOfCyborgsAt(row + 1, col + 1) +
            a.numberOfCyborgsAt(row + 1, col - 1) +
            a.numberOfCyborgsAt(row - 1, col + 1) +
            a.numberOfCyborgsAt(row - 1, col - 1);
        if (cyborgCount < minCyborgCount)
        {
            bestDir = i;
        }
    }
